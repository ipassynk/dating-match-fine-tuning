...
def generate_dating_pairs(core_domain, positive_actions, negative_actions):
    ...
    core_domain_interests = [{"category": "interests", "value": value} for value in core_domain["interests"]]
    ...
    for i in range(10):
        ...
        dating_pairs.append(InstructionalPair(Label.COMPATIBLE, Gender.MALE, 
            ...
            choice["category"], choice["value"], 
            Gender.FEMALE, 
            positive_actions[random.randint(0, len(positive_actions) - 1)], choice["category"], choice["value"]))
        dating_pairs.append(InstructionalPair(Label.COMPATIBLE, Gender.FEMALE, 
            negative_actions[random.randint(0, len(negative_actions) - 1)], 
            choice["category"], choice["value"], 
            Gender.MALE, 
            negative_actions[random.randint(0, len(negative_actions) - 1)], choice["category"], choice["value"]))

    # Incompatible pairs (shared negative preferences)
    for i in range(10):
        choice = random.choice(core_domain_lifestyle)
        dating_pairs.append(InstructionalPair(Label.INCOMPATIBLE, Gender.MALE, 
            "I am", 
            choice["category"], choice["value"], Gender.FEMALE, 
            "I hate people who are", choice["category"], choice["value"]))

    # Dealbreaker scenarios
    for i in range(10):
        choice1 = random.choice(core_domain_dealbreakers)
        dating_pairs.append(InstructionalPair(Label.INCOMPATIBLE,
            Gender.MALE, 
            positive_actions[random.randint(0, len(positive_actions) - 1)], 
