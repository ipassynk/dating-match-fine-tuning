...
def generate_dating_pairs(core_domain, positive_actions, negative_actions):
    ...
    for i in range(10):
        ...
        dating_pairs.append(InstructionalPair(Gender.MALE, 
            positive_actions[random.randint(0, len(positive_actions) - 1)], choice1["category"], choice1["value"], Gender.FEMALE, 
            negative_actions[random.randint(0, len(negative_actions) - 1)], choice2["category"], choice2["value"]))

    #Incompatible pairs (shared negative preferences)
    for i in range(10):
        choice1 = randon.choice(activities_interests);
        dating_pairs.append(InstructionalPair(Gender.MALE, 
            positive_actions[random.randint(0, len(positive_actions) - 1)], choice1["category"], choice1["value"], Gender.FEMALE, negative_actions[random.randint(0, len(positive_actions) - 1)], choice2["category"], choice2["value"]))
