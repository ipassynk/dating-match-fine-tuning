...
class InstructionalPair:
    def __init__(self, gender1, text1, gender2, text2):
        self.value = value
            ...
            for lifestyle in core_domain["lifestyle"]:
                for interest in core_domain["interests"]:
                    for activity in core_domain["activities"]:
                        dating_pairs.append(InstructionalPair(Gender.MALE, positive_actions[random.randint(0, len(positive_actions) - 1)], Gender.FEMALE, negative_actions[random.randint(0, len(negative_actions) - 1)]))
                        dating_pairs.append(InstructionalPair(Gender.FEMALE, positive_actions[random.randint(0, len(positive_actions) - 1)], Gender.MALE, negative_actions[random.randint(0, len(negative_actions) - 1)]))
    return dating_pairs
